...
def calculate_payment_status(monthly_rent: float, amount_paid: float) -> tuple[float, str]:
...
class PaymentService:
    ...
    def list_payments(self, month: str | None = None, customer_id: str | None = None) -> list[dict]:
        ...
        return sorted([self._attach_customer(p) for p in payments], key=lambda p: p.get("month", ""), reverse=True)

    def get_monthly_rows(self, month: str) -> list[dict]:
        active_customers = [c for c in self.repo.list("customers") if c.get("status") == "Active"]
        payments = {p.get("customer_id"): p for p in self.repo.list("payments") if p.get("month") == month}
        rows = []
        for customer in active_customers:
            payment = payments.get(customer["id"])
            rent = float(payment.get("monthly_rent") if payment else customer.get("monthly_rent", 0))
    ...
    def create_payment(self, payload: PaymentCreate) -> dict:
        ...
        existing = [
            p for p in self.repo.list("payments")
            if p.get("customer_id") == payload.customer_id and p.get("month") == payload.month
        ]
        data = payload.model_dump()
        try:
            remaining, status = calculate_payment_status(payload.monthly_rent, payload.amount_paid)
    ...
    def update_payment(self, payment_id: str, payload: PaymentUpdate) -> dict:
        ...
        try:
            remaining, status = calculate_payment_status(float(merged["monthly_rent"]), float(merged["amount_paid"]))
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        data.update({"remaining_amount": remaining, "payment_status": status, "updated_at": utc_now_iso()})
        return self.repo.update("payments", payment_id, data)

    def _attach_customer(self, payment: dict) -> dict:
        if payment.get("customer_name"):
            return payment
        customer = self.repo.get("customers", payment.get("customer_id", ""))
        if customer:
